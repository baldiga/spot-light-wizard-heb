...
const OutlineConfirmation = () => {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dir-rtl">
  ...
  return <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 dir-rtl">
      <div className="max-w-4xl mx-auto">
        ...
        {error && <Alert variant="destructive" className="mb-6 text-right">
        ...
        <div className="space-y-6 mb-8">
          {chapters.map((chapter, index) => <ChapterEditor key={chapter.id} chapter={chapter} chapterNumber={index + 1} />)}
        </div>
          ...
          <CardContent className="pt-6">
            <div className="text-right">
              ...
              <div className="space-y-4">
                {salesSteps.map((step, idx) => <div key={idx} className="bg-gray-50 p-4 rounded-lg border border-gray-100 text-right">
        ...
        <div className="flex justify-between">
          <Button variant="outline" onClick={() => navigate('/create')} className="border-whiskey text-whiskey hover:bg-whiskey/10">
            חזרה לעריכה
          </Button>
          <Button onClick={handleConfirm} className="bg-whiskey hover:bg-whiskey-dark text-white">אישור התוכן והמשך לסיכום המלא</Button>
        </div>
      </div>
    </div>;
};
